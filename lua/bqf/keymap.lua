local M = {}
local api = vim.api

local config = require('bqf.config')

local func_map

local action_funcref = {
    open = {mode = 'n', module = 'jump', funcref = 'open(false)'},
    openc = {mode = 'n', module = 'jump', funcref = 'open(true)'},
    split = {mode = 'n', module = 'jump', funcref = 'split(false)'},
    vsplit = {mode = 'n', module = 'jump', funcref = 'split(true)'},
    tab = {mode = 'n', module = 'jump', funcref = 'tabedit(false)'},
    tabb = {mode = 'n', module = 'jump', funcref = 'tabedit(true)'},
    ptogglemode = {mode = 'n', module = 'previewer.preview', funcref = 'toggle_mode()'},
    ptoggleitem = {mode = 'n', module = 'previewer.preview', funcref = 'toggle_item()'},
    ptoggleauto = {mode = 'n', module = 'previewer.preview', funcref = 'toggle()'},
    pscrollup = {mode = 'n', module = 'previewer.preview', funcref = 'scroll(-1)'},
    pscrolldown = {mode = 'n', module = 'previewer.preview', funcref = 'scroll(1)'},
    pscrollorig = {mode = 'n', module = 'previewer.preview', funcref = 'scroll(0)'},
    prevfile = {mode = 'n', module = 'qdo', funcref = 'nav_file(false)'},
    nextfile = {mode = 'n', module = 'qdo', funcref = 'nav_file(true)'},
    prevhist = {mode = 'n', module = 'qdo', funcref = 'nav_history(false)'},
    nexthist = {mode = 'n', module = 'qdo', funcref = 'nav_history(true)'},
    stoggleup = {mode = 'n', module = 'qdo', funcref = 'sign_toggle(-1)'},
    stoggledown = {mode = 'n', module = 'qdo', funcref = 'sign_toggle(1)'},
    stogglevm = {mode = 'x', module = 'qdo', funcref = 'sign_vm_toggle()'},
    stogglebuf = {mode = 'n', module = 'qdo', funcref = 'sign_toggle_buf()'},
    sclear = {mode = 'n', module = 'qdo', funcref = 'sign_clear()'},
    filter = {mode = 'n', module = 'filter.base', funcref = 'run()'},
    filterr = {mode = 'n', module = 'filter.base', funcref = 'run(true)'},
    fzffilter = {mode = 'n', module = 'filter.fzf', funcref = 'run()'}
}

local function init()
    func_map = config.func_map
    vim.validate({func_map = {func_map, 'table'}})
end

local function funcref_str(tbl_func)
    return ([[<Cmd>lua require('bqf.%s').%s<CR>]]):format(tbl_func.module, tbl_func.funcref)
end

function M.buf_map()
    for action, keymap in pairs(func_map) do
        local tbl_func = action_funcref[action]
        if tbl_func and not vim.tbl_isempty(tbl_func) and keymap ~= '' then
            api.nvim_buf_set_keymap(0, tbl_func.mode, keymap, funcref_str(tbl_func), {nowait = true})
        end
    end
end

init()

return M
