...
local function handler(qwinid, ret)
    ...
    if #selected_index == 1 then
        ...
        if action == 'tabedit' then
            ...
            jump.tabedit(false, qwinid, idx)
        elseif action == 'split' then
            jump.split(false, qwinid, idx)
        elseif action == 'vsplit' then
            jump.split(true, qwinid, idx)
        elseif not action then
            jump.open(true, qwinid, idx)
        end
    end
...
function M.prepare(qwinid, pid)
    local line_count = api.nvim_buf_line_count(api.nvim_win_get_buf(qwinid))
...
function M.run()
    ...
    cmd(('au BqfFilterFzf FileType fzf ++once %s'):format(
        ([[lua require('bqf.filter.fzf').prepare(%d, %s)]]):format(qwinid, tostring(pid))))
